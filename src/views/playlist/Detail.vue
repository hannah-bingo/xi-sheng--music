      ...
      <div class="detail-container">
        <div class="detail-main">
          ...
          <div class="song-main">
            ...
            <template v-if="isLoading && isLogin">
              <Loading />
            </template>
            <template v-else>
...
import songList from "@components/common/Play-list";
  ...
  methods: {
    ...
    async getAllSongs(ids) {
      ...
      for (let i = 0; i < sliceArr.length; i++) {
        ...
        this.isLoading = true;
        const { data: res } = await this.$http.songDetail({
          ids: arrs.join(","),
    ...
    async subPlayList(item) {
      const { data: res } = await this.$http.subPlayList({
        id: item.id,
