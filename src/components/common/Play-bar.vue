          ...
          <el-row class="play-bar-inside">
            ...
            <el-col :span="7" class="bar-l">
              ...
              <div class="bar-name">
                ...
                <p>
                  <router-link :to="{ path: '/artist/singer', query: { id: author.id } }" class="song_author" v-for="(author, k) in curSongInfo.singer" :key="author.name">{{ k !== 0 ? ' / ' +author.name : author.name }}</router-link>
                </p>
              </div>
              <div class="bar-time">
            ...
            <el-col :span="9" class="bar-m">
              ...
              <div class="bar-control">
                <i class="iconfont icon-audio-prev" @click.stop="audioHandler('prev')"></i>
...
export default {
  ...
  methods: {
    ...
    // 手动 切换歌曲
    changeSong (type) { //type: pre/next  上一首/下一首
...
.play-bar-inside {
  .bar-l {
    ...
    .bar-cover-img {
        ...
        height: 50px;
        border-radius: 4px;
        box-shadow: 0 0 15px 5px rgba(#ff641e,.15);
    } 
    .bar-name {
        flex: 1;
        font-size: 0;
